...
- [x] D-226 local transport: bound source-audit enumeration globally before whole-directory allocation; close readers on refusal. Actual20020-extra-file Linux fixture reads20001 entries, preserves all files and stops before mutation; old code reads20046 and fails the intended assertion.34 deployment CLI scenarios pass. Added evidence-based next acceptance stages in docs/A1-NEXT-ACCEPTANCE.md; public transition remains unimplemented/unapproved.
- [x] D-225 exact-source792fada acceptance completed01:15:49 UTC:47 commands,28 local groups and33 deployment scenarios. All log/report hashes and local-only plan verified01:19:02. D-226 supersedes this historical candidate and requires a fresh final freeze.

- [x] D-225 local acceptance correction: busy maintenance deadlines may expire before or during the final request. Both paths must fail without a success receipt, extra mutation or changed active-work state. Added deterministic stalled-poll case;34 CLI cases pass on Windows/Linux and false-drain negative fails. Production maintenance client unchanged. D-224 full acceptance failed at the prior timing-sensitive assertion; final exact-source acceptance must rerun. Architecture guide now distinguishes supported console/current CLI from legacy fixed-ewoq dev binary.
- [x] D-224 local backup tooling: source and verification walks read bounded directory entries and close handles on refusal, avoiding whole-directory allocation before limits. Actual4096-file source and4096-extra backup controls pass on Windows/Linux; old source and restored-old-verifier negatives fail at intended assertions.43 CLI/8–9 file-fault controls remain green. No scope/schema change or public installation; see docs/CONSOLE-BACKUP.md.
- [x] D-223 local experiment:5 real nodes/3 freshly created L1s,55 signed EVM transfers, actual missing-node and untracked-L1 negatives,4-node operation and complete catch-up after return. Six resource samples per0/1/3L1 phase: median total node memory244.32/342.605/528.40MiB. Exact cold-built binaries/genesis checked inside every node; all5 exit0/noOOM and evidence retained. Manually driven prototype setup faults documented; no production installer, WAN/Byzantine/capacity claim or public change. See docs/PRIVATE-NETWORK-VERIFICATION-2026-09-07.md.
- [x] D-222 local backup tooling: schema2 preserves exact temporary L1 genesis bytes, including corrupt/outside-band files, and requires root presence/absence observations. Legacy schema1 explicitly refused without migration.43 CLI checks pass on Windows/Linux with8/9 real-file fault controls; removing scope reproduces omitted real genesis and fails the expected-copy assertion. No public backup, restore or legacy adoption; see docs/CONSOLE-BACKUP.md.
- [x] D-221 local/read-only tooling: bounded selected legacy artifact inventory and exact committed-genesis comparison, with unchanged snapshots and no recovery authorization.93 Windows/94 Linux controls pass; three copied-source negatives reject false genesis/stability/deadline acceptance. Streamed tested code read-only on-server23:41 UTC:11 current artifacts match11 active chain genesis,55 outside-band files retained, snapshot unchanged. Shared strict read RPC preserves D-220 74-case behavior. This is not a backup, drain or bootstrap; see docs/LEGACY-ARTIFACTS.md.
- [x] D-220 local/read-only tooling: bidirectional P-chain registration versus selected ledger inventory, actual C/X aliases, repeated network/inventory/file identity, strict bounded inputs/RPC and no recovery authorization.74 actual CLI/file/HTTP cases pass on Windows/Linux; two removed-check negatives fail correctly. Actual public comparison23:16 UTC aligned13 registrations =2 primary+11 active,0 unlisted. This does not prove absence of pending/subnet-only operations or legacy drain; see docs/CHAIN-INVENTORY.md.
- [~] D-219: actual console readiness binds captured startup configuration, fresh expected network/parent identity, readable ledger, no unresolved creation and drained process identity. Deployment refuses configuration changes after backup or a wrongly configured replacement.40 client cases and31 actual-console cases pass on Windows/Linux;22 RPC cases and33 Docker deployment scenarios pass. Two removed/replaced-check negatives fail for the intended reason;26 local groups pass. No public deployment or consensus/capacity claim; see docs/CONSOLE-READINESS.md.
- [~] D-218: replaced copy-first deployment with exact-source validation, source/orphan audit, locked pause/drain, verified frozen upload, bounded backup, separate lockfile dependencies, per-file replacement and targeted paused restart. Apply retains pause/lock; separately reviewed resume requires public gates.28 actual Linux CLI cases and24 local groups pass. Fixed observed HTTP connection reuse across restart;33 client cases and a removed-header negative pass. Public adoption remains blocked on legacy API404 and undeclared helper ownership; no public mutation.
- [x] D-217 local backup tooling: proposed-release-bound old code and selected console state, absence/topology records, bounded exclusive copies and independent verification. 33 actual CLI cases pass on Windows/Linux; four/five real-file failure controls retain incomplete evidence. Integrated into D-218 controller; no public backup, restore or deployment.
...
- [~] **P-106 — Tách `local-net/console/server.mjs`** — 8 lượt tách + 2 lượt CỔNG (sổ · route) (2.927 dòng, tệp lớn nhất của core) thành mô-đun theo trục
      ...
      **Ca đỏ:** quên khai một mô-đun mới trong `manifest-deploy.json` ⇒ `check-deploy-imports` **đỏ và nêu tên tệp**.
      🔵 `08/09` chiều→tối (autopilot, D-249 → D-258) — **2.927 → 2.594 dòng**, tám lượt tách, **mỗi lượt có ca đỏ thật**
      ...
      *"có phải số không"* dưới dạng **networkID 0**, bị đem so với băng, và quay về thành **"LỆCH THẾ HỆ:
      ...
      ✅ **Lượt 8 — `console-state.mjs`** (hợp đồng router + lọc bí mật, **22 ca**). Lý do dời: **cả hai đều
      hỏng trong IM LẶNG**. Một bản đồ phân công sai **không ném lỗi** — nó dựng một router gửi `/ext/bc/<id>/*`
      tới node **không phục vụ** chain đó, và triệu chứng là một chain *"không tồn tại"* nhìn từ ngoài trong khi
      bên trong hoàn toàn khoẻ. Bản đồ đó là một **TỆP** chứ không phải mã dùng chung, chính vì đầu kia của nó là
      Caddy — nằm sau luật cứng #4.
      🔴 `redactSecret` **TỪ CHỐI** một bí mật rỗng thay vì lọc không có gì: `"abc".split("").join("<K>")` ra
      `"a<K>b<K>c"` — **kim rỗng không khớp KHÔNG GÌ, nó khớp GIỮA MỌI KÝ TỰ**, và câu log thành rác đúng lúc có
      người đang đọc nó để tìm lỗi. Trong console hôm nay điều đó **không xảy ra được** (`requireSecret` thoát nếu
      chuỗi < 16 ký tự) — nhưng bảo đảm ấy nằm cách lời gọi **ba trăm dòng**, và một hàm mà độ an toàn phụ thuộc
