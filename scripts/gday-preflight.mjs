...
const MANUAL_TASKS = [
  ...
  ["AFTER the network is up", "🔴 **B-13(b) — the OFFSET IS CHOSEN: `--offset-ms 3000`, measured on g1 2026-09-02** (skew dev↔node `+29ms ±726`, worst case node 697ms slow, floor covers it ~4x; `node scripts/check-clock-skew.mjs`). Two things remain for **2026-09-09**, and the second is not about clocks: **(1)** re-run the measurement while the chain is **PRODUCING BLOCKS** and prefer the `block.timestamp` source — that is the quantity the ceremony literally compares, and today it was unusable because the chain is idle (height 22, newest block 7,338s old). The tool now refuses a stale block instead of formatting it into an offset. **(2)** 🔴 **The C-Chain does not produce empty blocks**, so `block(Eva) + 9` — where the 9S Union message is anchored — **may never arrive on a quiet chain** (`docs/block-adam/CANON.txt`). Decide BEFORE the day: restart the heartbeat pump (⚠️ it refuses to start until `HEARTBEAT_STOP_AFTER`, currently `2026-09-01T00:00:00Z`, is moved — see the heartbeat task) or script nine deliberate filler transactions. ✅ The wording half is MOOT and that is measured, not assumed: the four engraved documents (`docs/engrave/CANON.txt`, 1,142 bytes, frozen on G-day) state **no timestamp at all**, so nothing engraved depends on this offset. ~~Original: measure clock skew across the 9 nodes, then pick `--offset-ms` for Block Adam.~~"],
