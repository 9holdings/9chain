...
export const ko: Dict = {
  ...
  validators: {
    title: "검증 노드를 운영하기",
    desc: "우리 홈페이지에 적힌 문장 — 검증자 중 아홉이 한 대의 머신, 한 곳의 제공업체에서 돌아간다 — 은 이 네트워크의 정직한 약점이고, 남는 머신을 가진 외부의 누군가만이 그것을 고칠 수 있습니다. 이 페이지는 그 대가가 무엇인지, 그리고 무엇을 돌려주지 않는지를 말합니다.",
    liveTitle: "지금의 검증자 집합",
    liveTotal: "검증자",
    liveConnected: "연결됨",
    liveMinBond: "최소 자기 담보",
    liveAtMinimum: "최소 담보로 스테이킹",
    measuredNote: "이 페이지가 열릴 때 네트워크에서 읽은 값이며, 손으로 적은 값이 아닙니다. 최소 담보는 노드 바이너리에 컴파일되어 있습니다 — 이 네트워크가 만들어지기 몇 시간 전까지 25,000이었으므로, 기억에 의존해 그 숫자를 적는 페이지는 재구축 한 번이면 돈에 대해 틀린 말을 하게 됩니다.",
    costTitle: "무엇이 드는가",
    costMachine: "계속 켜져 있는 머신과, 외부에서 9651 포트에 도달할 수 있는 공개 주소. 신청도, 허용 목록도, 프로토콜 차원의 허가 관문도 없습니다 — 제네시스에서 운영자 역할은 누구에게도 부여되지 않았으므로 잔액이 있는 계정이면 누구나 참여할 수 있습니다.",
    costBond: "선택한 기간 동안 잠기는 자기 담보: 최소 24시간, 최대 365일.",
    faucetTitle: "LOVE9는 어디서 오는가, 그리고 산수 속의 함정",
    faucetDesc: "포싯이 자금 조달의 전부입니다 — 신청할 것도, 부탁할 사람도 없습니다. 그런데 아홉 번의 요청은 정확히 담보액이 되고, 정확히 담보액으로는 부족합니다: 잔액을 C-Chain에서 X-Chain으로, 다시 P-Chain으로 옮기는 트랜잭션과 스테이크를 제출하는 트랜잭션의 수수료가 같은 잔액에서 나가기 때문입니다. 열 번의 요청과, IP당 한도가 풀릴 때까지 최대 한 시간의 대기를 예산에 넣으세요. 이 이야기를 끝이 아니라 여기서 하는 이유는, 우리 가이드의 이전 판이 \"아홉이면 된다\"고 써 놓고 삼백 줄 뒤에야 스스로 정정했기 때문입니다.",
    getTitle: "무엇을 얻는가",
    getRewards: "보상에는 기간 전체에서 80%의 가동률이 필요합니다 — Avalanche 메인넷보다 의도적으로 느슨합니다. 커뮤니티의 하드웨어는 데이터센터 하드웨어가 아니니까요.",
    getEnd: "기간이 끝나며 자동으로 갱신되는 것은 없습니다. 만료 시 스테이크는 돌아옵니다. 자신의 종료 시각은 종이에 계산하지 말고 체인에서 읽으세요.",
    getPrivacy: "RPC를 공개하도록 강제하는 것은 없으며, 9650 포트는 아예 열지 않기를 권합니다. 당신의 노드는 당신 것입니다.",
    honestTitle: "이것이 지불하지 않는 것",
    honest1: "LOVE9는 테스트 토큰입니다. 여기서도 다른 곳에서도 가치가 없고, 아무도 사지 않으며, 나중에 무엇인가로 바뀐다는 약속도 없습니다.",
    honest2: "A1은 테스트넷이며 이미 두 번 블록 0부터 다시 만들어졌습니다. 또 그런 일이 생기면 스테이크도, 보상도, 노드 신원도 함께 사라집니다. 우리가 약속하는 것은 미리 알리는 것과 무엇이 사라지는지 분명히 말하는 것 — 약속은 그게 전부입니다.",
    honest3: "가정용 공유기 뒤에서 노드는 스스로 연 연결만으로 부트스트랩하고 검증하며, 밖에서는 아무도 닿을 수 없는데도 아주 건강해 보입니다. 첫 외부 검증자는 그렇게 가동률 14%로 임기를 마치고 아무것도 얻지 못했습니다. 9651 포트를 포워딩하고, 공개 주소는 그 포워딩이 응답하는 주소로 설정하세요.",
    stepsTitle: "여섯 단계의 경로",
    step1: "소스를 받아 포크를 다시 빌드한 뒤 트리 해시를 직접 대조하세요 — 그리고 일부러 틀린 입력이 실패하는지도 확인하세요. 그래야 첫 번째 대조가 의미를 가집니다.",
    step2: "노드 이미지를 빌드하고, 어떤 commit에서 빌드했는지 각인하세요.",
    step3: "제네시스와 부트스트랩 주소를 받고, 무엇이든 실행하기 전에 제네시스 해시를 검증하세요.",
    step4: "노드를 실행하세요. 그 신원은 디스크의 파일 세 개입니다: 잃어버리면 당신의 담보는 더 이상 존재하지 않는 노드의 것이 됩니다.",
    step5: "네트워크 이름과 체인 ID를 되읽어 올바른 체인에 있는지 확인하세요. 200을 믿지 마세요.",
    step6: "LOVE9를 P-Chain으로 옮긴 뒤 스테이킹하세요 — 결과는 도구의 출력이 아니라 체인에서 확인하세요.",
    guideCta: "전체 가이드, 모든 명령",
    issuesCta: "문제 신고",
    issuesNote: "이슈 트래커가 창구이며, 공개인 것은 의도된 것입니다: 검증자의 문제는 거의 언제나 다른 사람도 겪을 문제이고, 사적으로 준 답은 한 사람만 돕습니다. 결론이 아니라 측정한 것을 알려주세요.",
  },
  docs: {
    title: "문서",
    desc: "A1에서 무언가를 하는 방법에 대해 적어 둔 모든 것 — 체인을 만드는 법, 검증 노드를 운영하는 법, 그리고 이 프로젝트가 무엇을 위한 것인지. 각 문서는 실제로 있는 자리로 연결되므로, 읽는 것이 곧 수정되는 사본입니다.",
    langNote: "각 문서는 해당 행에 표시된 언어로 되어 있으며, 문서 자체는 번역하지 않습니다. 번역본은 누군가 원문의 명령을 고치기 전까지만 옳습니다. 그리고 틀린 채로 남는 것은 아무도 손대지 않는 사본입니다.",
    langLabel: "언어",
    alsoIn: "다른 언어",
    pdfLabel: "PDF",
    onSiteLabel: "이 사이트에서",
    opensGithub: "GitHub에서 열림",
  },
  nineYears: {
    title: "9년, 90억",
    lede: "2026-09-09, 두 문장이 9Chain의 첫 블록에 새겨지고 9년의 셈이 시작됩니다. 2035년이면 유엔은 지구에 90억에 가까운 사람이 살 것으로 봅니다. 그 9년의 목표는 그들 한 사람 한 사람이 자기 블록체인을 갖는 것입니다.",
    oneLine: "내 AI는 허락을 구해야 한다 — 그리고 구했다는 사실을 기록하는 곳이 있다. 그곳은 내 것이다.",
    whatTitle: "지금 일어나는 일",
    what1: "매주 10억 명이 넘는 사람이 AI를 씁니다. 오늘 AI는 대답합니다. 내일은 행동합니다. 예약하고, 결제하고, 협상하고, 서명하고, 다른 사람의 AI 앞에서 당신을 대리합니다.",
    what2: "그래서 인류가 한 번도 답할 필요가 없던 질문이 생깁니다. 내 AI는 무엇을 해도 되는가, 그리고 증거는 누가 갖고 있는가? 오늘의 답은 당신에게 AI를 판 회사입니다. 권한도 그들이, 기록도 그들이, 문제가 생겼을 때 심판도 그들입니다.",
    what3: "“20달러 넘게 쓰지 마”라는 지시는 한도가 아닙니다. AI는 속을 수 있고, 설득될 수 있고, 명령을 주입당할 수 있습니다. 진짜 한도는 AI 바깥, AI가 고칠 수 없는 곳에 있으며, 그곳은 당신 것입니다.",
    promisesTitle: "다섯 가지 약속",
    promise1: "주권 — 누구도, 9Chain조차도 당신의 장부를 고치거나 지우거나 잠글 수 없습니다. 그 규칙은 당신이 정한 것입니다.",
    promise2: "영속 — 휴대폰을 잃어도 장부를 잃지 않습니다. 어떤 기기보다, 어떤 회사보다 오래 남습니다.",
    promise3: "검증 가능성 — 과거를 조용히 고쳐 쓸 수 없고, 누구나 그것을 확인할 수 있습니다. 당신의 증거가 상대편 손에 있지 않습니다.",
    promise4: "이동성 — 호스트도, 사업자도, 나라도 바꾸십시오. 이름과 기록은 함께 갑니다.",
    promise5: "상호운용 — 당신의 장부는 다른 사람의 장부와, 가게와, 공동체와 대화합니다.",
    promiseNot: "하지 않는 약속이 하나 있습니다. 당신의 체인이 영원히 돌아간다는 것. 그럴 필요가 없습니다. 당신이 잘 때 장부도 자고, 읽어야 하는 사람은 고정된 사본을 읽습니다.",
    constitutionTitle: "프롬프트가 아니라 헌법",
    constitutionDesc: "당신의 장부는 실행되는 계약을 담습니다. 어떤 AI가 당신을 대신해 서명할 수 있는지, 무엇을 할 수 있는지, 얼마까지, 언제까지, 그리고 언제 멈추고 다시 물어야 하는지. 누구의 허락도 없이 한 번의 터치로 철회됩니다. 모든 행위는 서명과 시각이 찍힌 영수증을 남깁니다.",
    constitutionStd: "지시가 아니라 계약인 이유는, 계약이야말로 AI가 말로 빠져나갈 수 없는 유일한 것이기 때문입니다. 세상이 이미 모여들고 있는 표준으로 쓰여 있어 어느 회사의 AI든 읽을 수 있습니다 — 이를 위해 우리만의 언어를 만들지 않았습니다.",
    treeTitle: "탑이 아니라 나무",
    treeDesc: "90억 개의 장부는 하나의 네트워크에 앉지 않습니다. 각 층은 아래층을 등록하고 고정하며, 증명을 다시 실행하는 대신 검증합니다. 그래서 나무는 뿌리를 굵게 하는 대신 가지를 더해 자랍니다.",
    treeRoot: "뿌리 — 작고, 오래가고, 일부러 지루합니다. 이름과 고정점을 지킵니다. 9Chain은 여러 뿌리 중 하나입니다.",
    treeTrunk: "줄기 — 한 지역, 한 나라, 공동체들의 연합.",
    treeBranch: "가지 — 동아리, 학교, 회사, 동네. 그 공동체가 스스로 운영합니다.",
    treeLeaf: "잎 — 당신의 것. 90억 장.",
    stagesTitle: "9년, 아홉 단계",
    stage2027: "진짜 공동체 하나에서 진짜 사람 백 명이 헌법 아래 일하는 AI와 함께.",
    stage2028: "세 대륙에서 열 개 공동체가 스스로 체인을 운영. 장부는 잃어버린 휴대폰을 넘어 남습니다.",
    stage2029: "열린 표준, 그리고 우리 것이 아닌 세 가지 구현.",
    stage2030: "9Chain이 운영하지 않으면서 이 표준으로 도는 첫 네트워크. 백만 개의 장부.",
    stage2031: "행동하는 AI가 기본이 되고, 권한의 뿌리가 없는 AI는 거절되기 시작합니다.",
    stage2032: "여러 뿌리, 하나의 표준. 표준의 거버넌스가 9Chain을 떠납니다.",
    stage2033: "장부가 신원 지갑, 휴대폰, 메신저, AI 플랫폼 안으로 들어갑니다.",
    stage2034: "휴대폰이 장부를 담아 출고됩니다. 10억 개가 실제로 쓰입니다.",
    stage2035: "한 사람에 한 체인. 첫 블록으로부터 9년.",
    stagesNote: "4년은 만들고 증명하는 시간, 다섯째 해에 세상이 그것을 필수로 만들고, 마지막 4년은 다른 사람들이 우리보다 멀리 가져가는 시간입니다. 마지막 부분은 위험이 아니라 계획입니다.",
    commitTitle: "하지 않겠다고 약속하는 것",
    commit1: "당신의 데이터를 보관하지 않습니다. 장부가 담는 것은 증거이지 당신의 사생활이 아닙니다.",
    commit2: "서류를 요구하지 않습니다. 신원은 당신과 당신 공동체 사이의 일입니다.",
    commit3: "삭제하지 않습니다. 이름도 장부도 기록도 영구적입니다. 잠들 뿐, 죽지 않습니다.",
    commit4: "장부를 가지려고 토큰을 사게 하지 않습니다. 표준은 열려 있고, LOVE9는 이 네트워크의 연료이지 입장권이 아닙니다.",
    commit5: "우리 AI를 만들지 않습니다. 어떤 AI든 같은 장부 위에서 당신을 도울 수 있도록 중립을 지킵니다.",
    joinTitle: "9년은 첫 사람들에게서 시작됩니다",
    joinDesc: "오늘 이것은 돌아가는 테스트 네트워크이며, 첫 외부 검증자와 첫 공동체 체인이 있습니다. 아래의 모든 것은 지금, 누구에게나 열려 있습니다.",
    fullDoc: "전체 문서",
  },
