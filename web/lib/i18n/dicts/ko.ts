import type { Tu } from '../en';

/**
 * 한국어 — 기계 번역이며 사람이 검수하지 않았습니다.
 * 원본 언어는 영어(`../en.ts`)이며, 차이가 있으면 영어판이 기준입니다.
 *
 * 🔴 다음 세 곳은 완화하지 마십시오: `reGenesis.*`(네트워크가 삭제됨),
 * `deChain.soatMoTa`(되돌릴 수 없는 문), `chainCuaToi.thuHoiY*`(철회해도 이름은 돌아오지 않음).
 * 이들은 "영구적으로", "변경할 수 없음"이라고 말합니다. 되돌릴 수 있다고 오해해 자산을 잃는
 * 일을 막기 위해서입니다.
 */
export const ko: Tu = {
  chung: {
    tenSanPham: '9Chain Testnet A1',
    moTaNgan: '9Chain의 공개 테스트넷 — Avalanche 엔진으로 구동되는 독립 네트워크',
    tagTitle: 'Avalanche 엔진 위의 독립 네트워크',
    viTuChoi: '지갑에서 요청을 거부하셨습니다. 변경된 것은 없습니다.',
    dangTai: '불러오는 중…',
    thuLai: '다시 시도',
    saoChep: '복사',
    daChep: '복사됨',
    dong: '닫기',
    moMenu: '메뉴 열기',
    dongMenu: '메뉴 닫기',
    chuyenSangToi: '다크 모드로 전환',
    chuyenSangSang: '라이트 모드로 전환',
    boQuaToiNoiDung: '본문으로 건너뛰기',
  },

  reGenesisXong: {
    luuUrl: '',
    luuSha256: '',

    bang: 'A1은 {ngay}에 재구축되었습니다. 그 이전에 만들어진 모든 잔액과 체인은 더 이상 존재하지 않습니다.',
    bangNut: '무슨 뜻인가요',
    nhan: '재구축됨',

    tieuDe: 'A1은 {ngay}에 재구축되었습니다',
    moTa:
      'A1 테스트 네트워크는 블록 0부터 다시 구축되었습니다. 그 날짜 이전에 만들어진 체인, 잔액, ' +
      '거래 내역은 더 이상 존재하지 않습니다 — 숨겨진 것이 아니라 사라졌습니다. ' +
      '이 페이지는 지금 보고 계신 것과 무엇을 해야 하는지 설명합니다.',

    thayGiTieuDe: '무엇이 보일까요',
    thayGi1:
      '지갑은 여전히 연결되고, 올바른 네트워크 이름과 동일한 Chain ID {chainId}를 그대로 표시합니다 — ' +
      '의도된 것입니다. 다만 잔액은 0이 됩니다.',
    thayGi2:
      '실행하셨던 모든 L1이 목록에서 사라졌습니다. 그 이름과 Chain ID는 다시 비었고, ' +
      '누구든 가져갈 수 있습니다.',
    thayGi3:
      '서명만 하고 한 번도 전송하지 않은 거래가 있다면 지금 전송하지 마십시오 — ' +
      '더 이상 존재하지 않는 네트워크의 거래입니다.',

    lamGiTieuDe: '무엇을 해야 하나요',
    lamGi1: '포시트에서 테스트 토큰을 다시 요청하십시오. 모든 사용자의 한도가 초기화되었습니다.',
    lamGi2:
      '각각의 L1을 지갑에서 제거하십시오 — 저마다 Chain ID를 갖고 있으며 이제 아무 곳도 가리키지 ' +
      '않습니다. 기본 A1 네트워크는 제거할 필요가 없습니다. 설정이 그대로이기 때문입니다.',
    lamGi3: '필요하다면 체인을 다시 실행하십시오. 예전 이름은 다른 사람이 가져갔을 수 있습니다.',

    luuTieuDe: '이전 네트워크 보관본',
    luuMoTa:
      '재구축 전 네트워크의 최종 상태를 내보내고 해시를 공개했습니다. ' +
      '확인하고자 하는 누구든 검증할 수 있습니다.',
  },

  reGenesis: {
    ngay: '2026-09-01',
    bang: 'A1은 {ngay}에 재구축됩니다 — 그 전에 만들어진 모든 체인, 잔액, 거래가 삭제됩니다.',
    bangNut: '자세히',
    nhan: '재구축 예정',

    tieuDe: 'A1은 {ngay}에 재구축됩니다',
    moTa:
      'A1 테스트 네트워크 전체가 블록 0부터 다시 구축됩니다. 그 날짜 이전에 만들어진 모든 것이 ' +
      '사라집니다 — 숨겨지는 것이 아니라 더 이상 존재하지 않게 됩니다. 이 페이지는 무엇이 사라지고 ' +
      '무엇을 해야 하는지 정확히 알려 드립니다.',

    viSaoTieuDe: '왜 재구축이 필요한가',
    viSao1:
      '네트워크의 genesis는 변경할 수 없습니다. 바로 그 점이 신뢰를 만듭니다 — 만든 사람을 포함해 ' +
      '누구도 블록 0에 기록된 숫자를 나중에 바꿀 수 없습니다.',
    viSao2:
      '그 대가는 이렇습니다. genesis 안의 숫자를 바꾸려면 네트워크를 처음부터 다시 만드는 것 외에 ' +
      '방법이 없습니다. A1은 총공급량을 9,000,000,000 LOVE9로 올렸고, 여기에 맞추기 위해 스테이킹 ' +
      '매개변수 전 범위를 다시 계산해야 했습니다.',
    viSao3:
      '이곳은 테스트넷이며, 재구축은 테스트넷에 허용된 일입니다. 사실 테스트넷이 존재하는 이유가 ' +
      '그것입니다. 이런 변경이 메인넷이 아니라 여기서 일어나도록 하는 것입니다.',

    matTieuDe: '무엇이 사라지나',
    matMoTa: '예외 없이 전부:',
    mat1: '사용자가 실행한 모든 L1. 아무 문제 없이 잘 돌아가는 체인도 포함됩니다.',
    mat2: '모든 LOVE9 잔액. 포시트에서 받은 토큰도 포함됩니다.',
    mat3: '모든 거래, 모든 블록, C-Chain·P-Chain·X-Chain의 전체 기록.',
    mat4: '모든 검증자와 모든 위임.',

    conTieuDe: '무엇이 남나',
    conMoTa:
      '삭제 전에 사라질 네트워크 전체를 내보내고 해시를 공개합니다. 그래야 기록이 검증 가능한 상태로 ' +
      '남습니다. 그것을 구동하던 네트워크가 사라진 뒤에도 무슨 일이 있었는지 확인할 수 있습니다. ' +
      '보관본 링크는 재구축 당일 이곳에 게시됩니다.',

    lamTieuDe: '무엇을 해야 하나요',
    lamTruoc: '재구축 전:',
    lam1:
      '데이터가 남아 있어야 하는 것을 지금 A1 위에 만들지 마십시오. 아이디어를 시험해 보는 것이라면 ' +
      '얼마든지 좋습니다 — 다만 지금의 체인을 저장소로 여기지는 마십시오.',
    lamSau: '재구축 후:',
    lam2:
      '추가하셨던 각각의 L1을 지갑에서 제거하십시오 — 그 체인들은 더 이상 존재하지 않으며, 그쪽을 ' +
      '가리키는 지갑은 그저 멈춰 있게 됩니다. 기본 A1 네트워크는 제거할 필요가 없습니다. 설정이 ' +
      '그대로이기 때문입니다.',
    lam3:
      '지갑에 아직 A1 네트워크가 없다면 설정을 직접 입력하지 말고 포시트 페이지의 버튼으로 추가하십시오.',
    lam4: '포시트에서 토큰을 다시 요청하고, 원하시면 체인을 다시 실행하십시오.',

    imLangTieuDe: '지갑은 경고해 주지 않습니다',
    imLangMoTa:
      '새 네트워크는 이전과 동일한 Chain ID {chainId}, 동일한 RPC 주소, 동일한 이름을 유지합니다. ' +
      '의도된 것입니다 — 이미 공개된 모든 문서와 안내가 그대로 맞도록 하기 위해서입니다. 그 대가로 ' +
      '지갑에는 방금 다른 네트워크에 연결되었다는 어떤 신호도 없습니다. 그래서 아래 두 가지가 ' +
      '조용히 일어납니다.',
    imLang1:
      '예전 설정의 지갑도 여전히 연결되고, 올바른 네트워크 이름을 보여 주며, 잔액을 0으로 표시합니다. ' +
      '그 숫자는 정확합니다. 예전 토큰은 더 이상 존재하지 않으며 숨겨진 것이 아닙니다. 네트워크를 다시 ' +
      '추가할 필요는 없습니다 — 포시트에서 새 토큰만 요청하십시오. 지갑이 멈춘 거래나 잘못된 순번을 ' +
      '알린다면 지갑에서 해당 네트워크의 활동 데이터를 지우십시오. 지갑은 이미 죽은 체인의 거래 수를 ' +
      '기억하고 있는데, 새 체인은 0부터 세기 때문입니다.',
    imLang2:
      '전송하지 않은 서명된 거래가 아직 있다면 폐기하십시오. Chain ID가 바뀌지 않았으므로 서명은 새 ' +
      '네트워크에서도 유효합니다. 지갑이 비어 있는 동안에는 실패하지만, 포시트에서 토큰을 받는 순간 ' +
      '사용 가능해지며 예상하지 못한 시점에 통과될 수 있습니다.',

    lapTieuDe: '또 일어날까요',
    lapMoTa:
      '가능합니다. A1은 여전히 테스트넷이며, 커뮤니티가 A1과 C1 사이에서 메인넷 방향을 정할 때까지는 ' +
      'genesis 안의 무언가를 바꿔야 할 때 네트워크를 재구축할 권리를 유지합니다. 저희가 약속하는 것은 ' +
      '미리 알려 드리는 것, 그리고 무엇이 사라지는지 분명히 말씀드리는 것입니다.',

    daXayRaTieuDe: '2026-08-27에 이미 한 번 재구축되었습니다',
    daXayRaMoTa:
      'A1은 아래 날짜 이전인 2026-08-27에 이미 한 번 재구축되었습니다. 그 전에 테스트 토큰을 보유하고 계셨다면 잔액은 지금 0입니다 — 이는 정상이며 지갑의 오류가 아닙니다. 사용자 체인은 하나도 사라지지 않았습니다. 목록에는 자동 테스트 체인만 있었습니다. 포시트에서 토큰을 다시 요청하십시오.',
    ngayLuuY: '날짜는 미뤄질 수 있습니다',
    ngayLuuYMoTa:
      '{ngay}이라는 날짜는 그 전에 이루어지는 go/no-go 점검에 달려 있습니다. 미뤄질 경우 침묵하지 않고 ' +
      '이 페이지의 날짜를 바꾸겠습니다.',
  },

  chanTrang: {
    dungThu: '사용해 보기',
    kham: '둘러보기',
    veDuAn: '소개',
    explorer: '9Scan-A1 익스플로러',
    trangChinh: '9Chain 메인 사이트',
    moTabMoi: '(새 탭에서 열림)',
    nhanNav: '푸터 링크',
    reGenesis: '네트워크 재구축 계획',
  },

  dieuHuong: {
    trangChu: '홈',
    faucet: '테스트 토큰 받기',
    console: '체인 실행',
    chainCuaToi: '내 체인',
    bang: 'A1 ↔ C1',
    danhBa: 'L1 목록',
    explorer: '익스플로러',
    banGiao: '9Scan-A1을 새 탭에서 열기',
  },

  trangChu: {
    nhanTestnet: '테스트넷 — 토큰에 실제 가치는 없습니다',
    nutChinh: '내 체인 실행하기',
    nutPhu: '먼저 테스트 토큰 받기',

    cTieuDe: 'A1에서 나만의 체인을 실행하세요',
    cPhu: '서명에 사용하는 지갑이 소유하는 나만의 L1이 테스트 네트워크에서 실제로 돌아갑니다. 약 3분 걸립니다.',
    cBangChuThich: '각 행은 A1에서 실제로 돌아가는 체인이며, 저마다 소유자가 있습니다.',
    cCot: '체인',
    cCotKieu: '유형',
    cCotChu: '소유자',
    cMacDinh: '시스템 기본값',
    cTrong: '아직 실행 중인 L1이 없습니다',
    cTrongMoTa: '첫 번째가 되실 수 있습니다. 체인이 올라오는 즉시 목록이 갱신됩니다.',

    tuTo: '10개 검증자 중 9개는 같은 서버, 같은 사업자에서 실행되고, 열 번째는 다른 사업자에서 실행됩니다. 프로토콜 수준에서는 분산되어 있고, 인프라 수준에서는 이제 막 시작되었습니다.',
    blockDungYen: 'Avalanche는 빈 블록을 만들지 않으므로, 아무도 거래하지 않을 때 블록 높이가 멈춰 있는 것은 정상입니다. 살아 있는지 판단하는 척도는 옆의 검증자 수입니다.',
  },

  soLieu: {
    tieuDe: '네트워크 가동 중',
    validator: '연결된 검증자',
    soL1: '실행 중인 L1',
    chieuCao: 'C-Chain 블록',
    dangDo: '네트워크 측정 중…',
    khongDo: '네트워크 통계를 읽지 못했습니다',
    khongDoMoTa: '페이지는 정상 작동합니다 — 이것은 상태 표시일 뿐입니다.',
  },

  loadTest: {
    badge: '부하 테스트',
    banner: '공개 부하 테스트를 진행 중입니다 — 초당 {tps}건의 거래로, 실제 사용자가 아니라 저희가 생성한 것입니다.',
    bannerLink: '실시간 수치 보기',
    title: '공개 부하 테스트',
    intro: 'A1은 실제 사용자가 매우 적은 신생 테스트 네트워크여서, 그대로 두면 블록이 거의 생성되지 않습니다. 네트워크가 끊임없이 작동하고 그 모습을 보실 수 있도록 저희가 일정한 거래 흐름을 만들고 있습니다. 이 트래픽은 저희 것입니다. 이는 사용량이 아니며 저희도 사용량으로 집계하지 않습니다 — 이를 보내는 모든 주소를 아래에 공개하니 빼고 보실 수 있습니다.',
    running: '실행 중',
    stopped: '현재 실행 중이 아님',
    stoppedWhy: '기록된 사유: {reason}',
    labelTps: '초당 거래 수',
    labelBlockHeight: 'C-Chain 블록',
    labelSecondsPerBlock: '블록당 초',
    labelTotal: '시작 이후 확정된 거래',
    labelUptime: '실행 시간',
    committedNote: '이 수치는 저희가 보내려 한 건수가 아니라 블록 자체에서 집계한 것입니다. 네트워크가 받아들였지만 블록에 포함되지 않은 거래는 여기에 집계되지 않습니다.',
    addressesTitle: '아홉 개의 발신 주소',
    addressesNote: '이 주소들에서 나온 모든 거래는 저희가 기계로 생성한 것입니다. 걸러내면 실제 활동을 확인할 수 있습니다.',
    measuring: '부하 테스트 상태를 읽는 중…',
    notMeasured: '부하 테스트 상태를 읽을 수 없습니다',
    notMeasuredMore: '페이지는 정상 작동합니다 — 이것은 상태 표시일 뿐입니다.',
  },

  deChain: {
    tieuDe: '내 체인 실행하기',
    moTa:
      '지갑이 소유하는 전용 L1입니다. 본인임을 증명하기 위해 한 번 서명하고, 내용을 확인하면 ' +
      '네트워크가 약 3분 만에 체인을 만들어 줍니다.',

    noiVi: '지갑 연결',
    dangNoi: '연결 중…',
    kyDeVao: '로그인',
    dangKy: '서명 대기 중…',
    viCuaBan: '내 지갑',
    laChuChain: '체인은 이 지갑의 소유가 됩니다. 주소는 서명에서 나옵니다 — 아무도 입력하지 않습니다.',
    khongCoVi: '이 브라우저에서 지갑을 찾을 수 없습니다. MetaMask를 설치하고 페이지를 새로고침하십시오.',
    tuChoiKy: '서명을 거부하셨습니다. 아무것도 생성되지 않았습니다.',
    doiVi: '다른 지갑 사용',

    nhanTen: '체인 이름',
    goiYTen: '예: MyChain',
    moTaTen:
      '문자, 숫자, 공백. 2–32자. 이 네트워크에서 한 번 사용된 이름은 다시 발급되지 않습니다 — ' +
      '철회된 체인의 이름이라도 마찬가지입니다.',
    tenXau: '이름에는 문자, 숫자, 공백만 사용할 수 있으며 길이는 2–32자여야 합니다.',
    nhanKieu: '체인 유형',
    moTaKieu: '한 번 선택하면 고정됩니다 — 체인의 genesis는 수정할 수 없습니다.',
    conCho: '{con}/{tong}개 자리 남음',
    hetCho: '남은 자리가 없습니다',
    hetChoMoTa:
      '현재 방식에서는 모든 검증자가 모든 L1을 추적하며, 16개를 넘는 서브넷을 선언한 노드는 프로토콜이 ' +
      '제외합니다. 이는 고정된 상한이며 올릴 수 없습니다. 체인을 철회하면 자리 하나가 반환됩니다.',
    soatLai: '제출 전 확인',

    soatTieuDe: '확인 — 이것은 되돌릴 수 없는 문입니다',
    soatMoTa:
      '실행된 L1의 genesis는 변경할 수 없습니다. 이 단계 이후에는 이름, 체인 유형, 소유자를 바꿀 수 ' +
      '없으며 — 철회하더라도 이름과 chain ID는 돌려받지 못합니다.',
    soatReGenesis:
      '누르기 전에 한 가지 더 알아 두십시오. A1은 {ngay}에 네트워크 전체를 재구축합니다. 오늘 실행하는 ' +
      '체인은 이전 네트워크와 함께 삭제됩니다 — 숨겨지는 것이 아니라 사라집니다.',
    soatTen: '체인 이름',
    soatKieu: '체인 유형',
    soatChu: '소유자',
    soatQuayLai: '돌아가서 수정',
    soatDongY: '확인했습니다 — 체인 실행',

    dangDe: '“{ten}” 체인을 실행하는 중',
    dangDeMoTa:
      '네트워크가 정족수를 잃지 않도록 노드를 하나씩 순서대로 재시작합니다 — 그래서 느리며, 이는 ' +
      '의도된 것입니다. 탭을 닫지 마십시오. 닫더라도 체인은 계속 만들어집니다.',
    conKhoang: '약 {phut}분 남음',
    dangChuanBi: '준비 중…',

    xongTieuDe: '완료 — “{ten}” 체인이 실행 중입니다',
    xongChainId: 'Chain ID',
    xongRpc: 'RPC',
    xongThemVi: '지갑에 체인 추가',
    xongDaThem: '지갑에 추가됨',
    xongKichHoat: '체인 활성화(블록 1 열기)',
    xongDaKichHoat: '활성화됨',
    xongDangKichHoat: '지갑 대기 중…',
    xongThemViLoi: '체인을 지갑에 추가하지 못했습니다. {chiTiet}',
    xongKichHoatLoi: '체인을 활성화하지 못했습니다. {chiTiet}',

    deTiep: '체인 하나 더 실행',
    loiDe: '체인을 실행하지 못했습니다. {chiTiet}',
    loiKhongRo: '작업이 끝난 뒤에도 체인이 목록에 나타나지 않았습니다.',
    luuYTieuDe: '새 체인에서의 첫 거래',
    luuYCachLam:
      '첫 거래의 가스 추정치를 믿지 마십시오. 블록 1을 여는 가장 저렴한 방법은 평범한 전송입니다 — ' +
      '아래 “체인 활성화”를 누르십시오.',
  },

  chainCuaToi: {
    tieuDe: '내 체인',
    moTa: '로그인에 사용한 지갑이 소유한 L1 목록입니다. 철회할 수 있지만 먼저 경고를 읽어 주십시오.',
    noiVi: '체인을 보려면 지갑을 연결하십시오',
    trongTieuDe: '이 지갑은 아직 체인을 갖고 있지 않습니다',
    trongMoTa: '하나 실행한 뒤 돌아오십시오 — 곧바로 여기에 표시됩니다.',
    trongNut: '내 체인 실행하기',

    cotChain: '체인',
    cotKieu: '유형',
    cotSong: '상태',
    cotViec: '',

    songDo: '검증자 {so}개',
    songDangDo: '측정 중',
    songKhongDo: '측정하지 못함',
    songGiaiThich: '블록 높이가 아니라 서브넷의 검증자 수로 측정합니다.',
    khongValidator: '검증자 0개',
    khongValidatorMoTa:
      '이 체인은 어떤 거래도 확정할 수 없습니다. 서브넷에 검증자가 없기 때문입니다. 그래도 RPC 호출에는 ' +
      '응답하고 지갑도 연결되므로, 겉으로 드러나는 다른 신호가 없습니다.',

    thongSo: '지갑 설정',
    themVaoVi: '지갑에 추가',
    daThemVaoVi: '추가됨',
    themViLoi: '지갑에 추가하지 못했습니다. {chiTiet}',

    thuHoi: '철회',
    thuHoiTieuDe: '“{ten}”을(를) 철회할까요?',
    thuHoiY1: '체인은 즉시 RPC 제공을 중단하고 공개 목록에서 사라집니다.',
    thuHoiY2:
      '철회해도 P-Chain의 서브넷은 삭제되지 않습니다 — 그곳에 만들어진 것은 이 네트워크가 도는 동안 ' +
      '제거할 수 없습니다. 또한 이미 이 체인을 추가한 사람들의 지갑에서 네트워크를 지워 주지도 않습니다.',
    thuHoiY3:
      '이름과 Chain ID는 예약된 채로 남으며 이 네트워크에서 누구에게도 다시 발급되지 않습니다. Chain ID를 ' +
      '다시 발급하면 예전 사용자의 지갑이 조용히 다른 사람의 체인을 가리키게 되기 때문입니다.',
    thuHoiY4: '그 대신 15개 자리 중 하나가 반환됩니다.',
    thuHoiGoNhan: '확인하려면 체인 이름을 정확히 입력하십시오',
    thuHoiSaiTen: '체인 이름과 일치하지 않습니다.',
    thuHoiXacNhan: '영구히 철회',
    thuHoiHuy: '취소',
    thuHoiDangChay: '“{ten}” 철회 중 — 약 3분',
    thuHoiXong: '“{ten}”을(를) 철회했습니다. {con}/{tong}개 자리 남음.',
    thuHoiLoi: '철회하지 못했습니다. {chiTiet}',
    thuHoiKhongRo: '작업이 끝난 뒤에도 체인이 목록에 남아 있습니다.',

    daThuHoi: '철회됨',
    daThuHoiMoTa: '이름과 Chain ID는 이 네트워크에서 예약된 채로 남습니다.',
  },

  bang: {
    tieuDe: 'A1 ↔ C1 — 비교',
    moTa:
      '9Chain은 같은 제품의 테스트넷 두 개를 나란히 운영하며, 차이는 엔진입니다. A1은 Avalanche 엔진, ' +
      'C1은 Cosmos 엔진입니다. 이 표는 두 방향 사이의 절충을 기록하며, 누구든 반박할 수 있도록 ' +
      '공개했습니다 — C1 쪽은 아직 실측값이 없습니다.',

    tuChamTieuDe: '아래 점수는 팀이 자체 평가한 것이며 독립적으로 측정된 것이 아닙니다',
    tuChamMoTa:
      '"측정 방법" 열은 각 기준을 어떻게 확인했는지 알려 줍니다. 날짜가 적힌 측정이 없는 기준은 데이터가 ' +
      '아니라 설계적 판단입니다. 가중치는 직접 정하십시오 — 점수는 그에 따릅니다.',

    cotSo: '#',
    cotTieuChi: '기준',
    cotLoai: '유형',
    cotA1: 'A1',
    cotC1: 'C1',
    cotTrongSo: '가중치',
    loaiKienTruc: '설계',
    loaiSong: '실시간 데이터',

    tongDiem: '입력하신 가중치 기준 총점',
    hoaNhau: '동점',
    dangDan: '우세',

    soLieuTieuDe: '실시간 데이터',
    a1Validator: 'A1 — 연결된 검증자',
    a1Chain: 'A1 — 실행 중인 L1',
    a1Block: 'A1 — C-Chain 블록',
    c1Vang: 'C1 — 접속 불가',
    c1VangMoTa:
      'C1의 Cosmos REST URL(포트 1317)이 필요합니다. 표는 그대로 작동합니다. A1 쪽은 실시간 데이터이고, ' +
      'C1 쪽은 나머지 기준과 마찬가지로 설계적 판단입니다.',
    dangDo: '측정 중…',
    khongDo: '측정하지 못함',
  },

  faucet: {
    tieuDe: '테스트 토큰 받기',
    moTa:
      'A1 테스트넷의 LOVE9에는 실제 가치가 없습니다 — 시험하는 동안 가스를 낼 수 있도록 존재합니다. ' +
      '지갑 주소를 입력하시면 바로 보내 드립니다.',
    nhanDiaChi: '내 지갑 주소',
    goiYDiaChi: '0x… (16진수 40자)',
    nutXin: '토큰 보내 주세요',
    dangGui: '보내는 중…',
    danChoDiaChi: '토큰을 받을 지갑 주소를 붙여 넣으십시오. 아직 하지 않으셨다면 위의 “지갑에 네트워크 추가”를 누르십시오.',
    themMang: '지갑에 네트워크 추가',
    themMangXong: '지갑에 추가됨',
    themMangTuChoi: '지갑에서 거부를 누르셨습니다. 네트워크를 추가하시려면 다시 누르십시오.',
    themMangLoi: '지갑이 네트워크를 추가하지 못했습니다. 옆의 설정으로 직접 추가하시고 — 아래 줄을 팀에 보내 주십시오:',
    khongCoVi: '이 브라우저에서 지갑을 찾을 수 없습니다. MetaMask를 설치하고 페이지를 새로고침하십시오.',
    hanMucConLai: '남은 할당량',
    hanMucCachDoc: '{gio}시간당 {con}/{tong}회 요청',
    hanMucHet: '할당량을 모두 사용하셨습니다. {phut}분 뒤에 다시 시도하십시오.',
    hanMucKhongDoc: '할당량을 읽지 못했습니다 — 요청은 여전히 가능하지만 몇 번 남았는지는 알 수 없습니다.',
    thanhCong: '{diaChi}(으)로 {so} {kyHieu}를 보냈습니다',
    xemGiaoDich: '거래 보기',
    thongSoMang: '네트워크 설정',
    thongSoRpc: 'RPC',
    thongSoChainId: 'Chain ID',
    thongSoKyHieu: '기호',
    thongSoThapPhan: '소수 자릿수',
    thongSoExplorer: '익스플로러',
    thapPhanGiaiThich:
      'C-Chain이 EVM을 구동하기 때문에 지갑은 소수점 18자리를 표시합니다. P/X-Chain에서 LOVE9는 소수점 ' +
      '9자리로 셉니다. 하나의 코인, 두 개의 척도 — 서로 다른 두 토큰이 아닙니다.',
    loiChung: '보내지 못했습니다. {chiTiet}',
  },

  chonNgonNgu: {
    nhan: '언어',
    mayDich: '기계',
    mayDichGiaiThich: '베트남어판만 사람이 검수했습니다. 나머지 번역은 기계가 만든 것이라 틀릴 수 있습니다 — 영어판이 기준입니다.',
    chuaCo: '아직 없음',
  },

  loi: {
    khongKetNoi: '네트워크에 연결하지 못했습니다',
    khongKetNoiMoTa: '네트워크가 혼잡하거나 연결이 끊겼을 수 있습니다.',
    trongRong: '여기에는 아직 아무것도 없습니다',
  },

  khongThay: {
    ma: '404',
    tieuDe: '이 페이지는 존재하지 않습니다',
    moTa:
      '열어 보신 주소는 9Chain Testnet A1에 존재하지 않습니다. ' +
      '이름이 바뀌었거나, 복사하는 과정에서 URL의 일부 문자가 빠졌을 수 있습니다.',
    dayLaGi: '가장 많이 쓰이는 세 페이지:',
    nhanNav: '다음에 갈 곳',
    veTrangChu: '홈으로 돌아가기',
    diFaucet: '테스트 토큰 받기',
    diDeChain: '내 체인 실행하기',
    timGiaoDich: '거래나 주소를 찾고 계신가요? 해시를 확인하고 다시 시도하십시오.',
  },
};

export default ko;
