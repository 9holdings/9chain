...
export const zh: Dict = {
  ...
  validators: {
    title: "运行一个验证节点",
    desc: "我们首页上那句话——九个验证节点跑在同一台机器、同一个供应商——是这个网络真实的弱点，而一位手上有闲置机器的外部参与者，是唯一能修复它的人。本页讲清楚这要花什么代价，以及它不会回报什么。",
    liveTitle: "此刻的验证者集合",
    liveTotal: "验证节点",
    liveConnected: "已连接",
    liveMinBond: "最低自抵押",
    liveAtMinimum: "以最低额抵押",
    measuredNote: "本页加载时从网络读取，不是手写的。最低抵押额编译在节点二进制里——在本网络创建前几小时它还是 25,000，所以凭记忆引用这个数字的页面，离“对钱说错话”只差一次重建。",
    costTitle: "需要什么",
    costMachine: "一台保持开机的机器，以及一个外部可访问 9651 端口的公网地址。无需申请、无白名单、协议层没有许可门槛——创世时未向任何人授予运营角色，因此任何有余额的账户都能加入。",
    costBond: "一笔自抵押，按你选择的期限锁定：最短 24 小时，最长 365 天。",
    faucetTitle: "LOVE9 从哪来，以及这道算术里的陷阱",
    faucetDesc: "水龙头就是全部的资金来源——无需申请，也不必求人。但九次请求恰好等于抵押额，而恰好等于抵押额是不够的：把余额从 C-Chain 转到 X-Chain 再到 P-Chain、以及提交抵押的那些交易，手续费都从同一笔余额里出。请按十次请求来规划，并预留最多一小时等待每 IP 限额恢复。我们把这写在开头而不是结尾，因为我们自己指南的早期版本曾写“九次就够”，却在三百行之后才更正。",
    getTitle: "你会得到什么",
    getRewards: "奖励需要在整个期限内达到 80% 在线率——这比 Avalanche 主网宽松，是刻意的，因为社区的硬件不是数据中心的硬件。",
    getEnd: "期限结束后不会自动续期。到期时抵押退回；请从链上读取你自己的结束时间，而不是在纸上推算。",
    getPrivacy: "没有任何规定要求你暴露 RPC 接口，我们更希望你根本不要开放 9650 端口。你的节点是你的。",
    honestTitle: "这件事不会回报你什么",
    honest1: "LOVE9 是测试代币。它在这里没有价值，在别处也没有价值，没有人在买它，也没有任何承诺说它以后能换成什么。",
    honest2: "A1 是测试网，已经从第 0 块重建过两次。如果再发生一次，你的抵押、奖励和节点身份都会随之消失。我们承诺的是提前说明、并把损失讲清楚——承诺仅此而已。",
    honest3: "在家用路由器后面，节点靠自己发起的连接也能启动并验证，看上去完全健康，而外面没有人能连到它。第一位外部验证者正是这样以 14% 在线率结束了一整个期限，一无所获。请转发 9651 端口，并把公网地址设为该转发实际应答的地址。",
    stepsTitle: "路径，六步",
    step1: "取得源码并重建这个分叉，然后自己核对树哈希——同时验证一个故意写错的输入会失败，这样第一次核对才有意义。",
    step2: "构建节点镜像，把你所依据的 commit 打进去。",
    step3: "获取 genesis 和一个引导地址，在运行任何东西之前先校验 genesis 哈希。",
    step4: "运行节点。它的身份就是磁盘上的三个文件：丢了它们，你的抵押就属于一个不再存在的节点。",
    step5: "通过读回网络名称和链 ID 来确认你在正确的链上，而不是相信一个 200。",
    step6: "把 LOVE9 转到 P-Chain，然后抵押——并在链上核验结果，而不是看工具的输出。",
    guideCta: "完整指南，逐条命令",
    issuesCta: "报告问题",
    issuesNote: "issue 页面就是联系渠道，而且刻意是公开的：验证者遇到的问题几乎总会有别人也遇到，私下给出的答案只帮到一个人。请告诉我们你测到了什么，而不是你的结论。",
  },
  docs: {
    title: "文档",
    desc: "关于使用 A1 的全部记载：如何创建一条链、如何运行验证节点，以及这个项目是为了什么。每份文档都链接到它真正所在的位置，所以你读到的就是会被修改的那一份。",
    langNote: "每份文档使用其所在行标注的语言，我们不翻译文档本身。译本只在有人修改原文中的某条命令之前保持正确——而没人修改的那一份，就是会出错的那一份。",
    langLabel: "语言",
    alsoIn: "另有版本",
    pdfLabel: "PDF",
    onSiteLabel: "本站页面",
    opensGithub: "在 GitHub 打开",
  },
