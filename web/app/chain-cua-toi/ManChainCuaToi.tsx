...
import { layVi, noiVi, dangNhapSiwe, goiConsole, themL1VaoVi, choTienTrinhXong, type PhienVi } from '@/lib/ket-noi-vi';

type Chain = {
  name: string; chainId: number; subnetID: string; blockchainID: string;
  admin?: string; presetTen?: string; rpc?: string;
};
type TrangThai = { tran: number; chains: Chain[]; retired: Chain[]; viDangNhap: string | null };
type TienTrinh = { dangChay: boolean; buoc: MotBuoc[]; uocConLaiGiay: number };
...
export function ManChainCuaToi() {
  ...
  useEffect(() => {
    ...
    const doc = async () => {
      try {
        datTienTrinh(await goiConsole<TienTrinh>('/api/tien-trinh', phien.token));
  ...
  async function thucHienThuHoi(c: Chain) {
    ...
    if (conSong) {
      datLoiThuHoi(dien(vi.chainCuaToi.thuHoiLoi, {
        chiTiet: kq?.loi ?? loiPost ?? vi.chainCuaToi.thuHoiKhongRo,
      }));
    }
  ...
  if (chay) {
    return (
      <The className="mt-8 max-w-2xl p-5 md:p-6">
        ...
        <div className="mt-4">
          {tienTrinh?.buoc?.length ? (
            <CacBuoc
              buoc={tienTrinh.buoc}
              ghiChu={tienTrinh.uocConLaiGiay
                ? dien(vi.deChain.conKhoang, { phut: Math.max(1, Math.ceil(tienTrinh.uocConLaiGiay / 60)) })
                : undefined}
            />
          ) : (
    ...
    <div className="mt-8 flex max-w-3xl flex-col gap-6">
      ...
      {!cuaToi.length && !cuaToiDaThuHoi.length ? (
        <TrongRong
          ...
          hanhDong={
            <a href="/de-chain/" className="inline-flex h-11 items-center rounded-btn bg-gold px-4 text-sm font-semibold text-navy hover:bg-gold-hover">
                  ...
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div>
                      ...
                      <p className="mt-1 flex flex-wrap items-center gap-2 text-sm">
                        {c.presetTen && <Nhan>{c.presetTen}</Nhan>}
                        {v === undefined || v === 'dang' ? (
                  ...
                  {c.rpc && (
                    <div className="mt-3">
                      <Nut
                        ...
                        onClick={async () => {
                          try {
                            await themL1VaoVi({ chainIdHex: '0x' + c.chainId.toString(16), ten: c.name, rpc: c.rpc!, kyHieu: 'LOVE9' });
