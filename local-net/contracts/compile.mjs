/**
 * compile.mjs — biên dịch `AssetBridge.sol` thành artifact commit được.
 *
 ...
 *   node local-net/contracts/compile.mjs --solc <đường dẫn tới node_modules/solc>
 *
 * Kết quả: `local-net/lib/asset-bridge.mjs` (ABI + bytecode + dấu vân tay bản dịch).
 */
...
const nguon = {
  "AssetBridge.sol": { content: doc("AssetBridge.sol") },
...
const hd = kq.contracts["AssetBridge.sol"]["AssetBridge"];
...
const van = createHash("sha256").update(nguon["AssetBridge.sol"].content).digest("hex").slice(0, 16);

const ra = `/**
 * AssetBridge — artifact SINH TỰ ĐỘNG, đừng sửa tay.
 *
 * Nguồn : local-net/contracts/AssetBridge.sol  (sha256 16 ký tự đầu: ${van})
 * solc  : ${solc.version()}
 * dựng lại: node local-net/contracts/compile.mjs --solc <đường dẫn solc>
 */
...
`;
const dich = path.join(GOC, "..", "lib", "asset-bridge.mjs");
