...
# The fork sits three levels up from local-net/tools/k1/, exactly as go.work's `use` says.
FORK="$(cd ../../../upstream/avalanchego && (pwd -W 2>/dev/null || pwd))"; FORK="/${FORK#/}"; FORK="${FORK/C:/c}"; FORK="${FORK/c\//c/}"
# Mount both trees at paths that keep go.work's relative `use` directives valid.
MNT_KIT=/w/9chain-a1/local-net/tools/k1
MNT_FORK=/w/9chain-a1/upstream/avalanchego
