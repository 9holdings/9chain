/**
 * cau-test.mjs — M6.2 bước 2: TÀI SẢN đi từ L1 này sang L1 kia, có bằng chứng hai đầu.
 *
 ...
 *            node local-net/faucet/cau-test.mjs'
...
async function dungChain(ten, vi, nhan, daDe) {
  ...
  const { hd, diaChi } = await napHopDong(chu, CAU_TAI_SAN_ABI, CAU_TAI_SAN_BIN, `${nhan} nạp CauTaiSan`);
...
console.log(`   hợp đồng   : CauTaiSan, vân tay nguồn ${CAU_TAI_SAN_VAN_TAY_NGUON}`);
