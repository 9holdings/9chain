...
// Địa chỉ EVM được cấp phát + làm admin phí trên MỌI L1 do console đẻ ra.
//
...
// phí của mọi L1 vừa tạo. Đặt A1_L1_ADMIN=0x... để dùng địa chỉ của mình.
const EWOQ = "0x8db97C7cEcE249c2b98bDC0226Cc4C2A57BF52FC";
const L1_ADMIN = process.env.A1_L1_ADMIN || EWOQ;
if (!/^0x[0-9a-fA-F]{40}$/.test(L1_ADMIN)) {
  console.error(`FATAL: A1_L1_ADMIN không phải địa chỉ EVM hợp lệ: ${L1_ADMIN}`);
  process.exit(1);
}
...
// Tạo 1 L1: trả {name, subnetID, blockchainID, chainId, rpc}
async function createChain({ name, chainId }) {
  ...
  chainId = Number(chainId) || (9100 + state.chains.length);

  // 1) genesis EVM cho L1 này
  const tpl = JSON.parse(readFileSync(L1_TEMPLATE, "utf8"));
  tpl.config.chainId = chainId;
  tpl.config.feeManagerConfig = { adminAddresses: [L1_ADMIN], blockTimestamp: 0 };
  tpl.alloc = { [L1_ADMIN.slice(2)]: { balance: "0x295BE96E64066972000000" } };
  ...
  const chain = { name, subnetID, blockchainID, chainId, rpc: `${rpcBase}${rpcPath}`, createdAt: Date.now() };
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (req.method === "GET" && req.url === "/api/status") {
      ...
      return send(res, 200, { node: ver, cChainId: cid, chains: loadState().chains });
    }
